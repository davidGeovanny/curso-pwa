...
const enviarNotificacion = () => {
    ...
    const notification = new Notification('Hola Mundo', notificationOptions);
    ...
    if( Notification.permission === 'granted' ) {
        ...
        Notification.requestPermission( ( permission ) => {
            ...
            if( permission === 'granted' ) {
                ...
                // new Notification('Hola Mundo - if: granted');
                enviarNotificacion();
            }
        });
    }
...
// Get KEY
const getPublicKey = () => {
    return fetch('http://localhost:3000/api/key').then( resp => {
        return resp.arrayBuffer();
    }).then( newResp => {
        return new Uint8Array( newResp );
    });
};
...
btnDesactivadas.on('click', function( event ) {
...
btnActivadas.on('click', function( event ) {
    event.preventDefault();
...
const cancelarSuscripcion = () => {
    swRegister.pushManager.getSubscription().then( resp => {
        resp.unsubscribe().then( () => {
            return verificaSuscripcion( false );
        })
    });
}